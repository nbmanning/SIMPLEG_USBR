...
# Next Steps:
## REMOVE all that is not soy/maize mapping. Every other step can be
# done in processResults_SIMPLEG_2.R, no need to have it all here. 

## Finish US mapping, have updated Brazil, Cerrado, and World

## Use 'tidyterra' instead

## Make more reproducible by cleaning the beginning section
...
### Maize Map ####################
#### Post-Sim Cropland Area ####################
terra::plot(r_row %>% subset("new_LND_MAZ"),
            # type = "interval",
            # breaks = c(0, 1, 5, 10, 20, 25, 30, 35, 45, 50),
            type = "continuous",
            #breaks = c(0, 1, 5, 10, 20, 25, 30, 35, 45, 50),
            ...
            #col = brewer.pal(9, "YlGn"),
            main = paste("Post-Simulation Maize Area", pct_title),
            plg=list( # parameters for drawing legend
              title = "Area (ha) / Grid Cell",
              #title.cex = 2, # Legend title size
              #cex = 2 # Legend text size
              x = "bottomleft"
            )
...
# plot w ggplot & tidyterra

# # Available colors:"atlas", "high_relief", "arid", "soft", "muted", "purple", "viridi", "gn_yl", "pi_y_g", "bl_yl_rd", "deep"
# ggplot()+
#   geom_spatraster(
#     data = r_row %>% subset("new_LND_MAZ"), 
#     maxcell = Inf)+
...
#   labs(
#     fill = "Area",
#     title = paste("Post-Sim Maize Area", pct_title)
#   )+
#   #theme_minimal() +
#   theme(
...
#### Actual (Raw) Change in Cropland Area ####################
terra::plot(r_row %>% subset("rawch_MAZ"),
            #type = "interval",
            #breaks = c(-50, -25, -10, -1, -0.1, 0, 0.01, 0.1, 0.25, 0.5, 1),
            #col = brewer.pal(n = 11, name = "RdBu"),
            ...
            main = paste("Raw Change in Maize Area", pct_title),
            plg=list( # parameters for drawing legend
              title = "Area (ha)/ Grid Cell",
              #title.cex = 2, # Legend title size
              #cex = 2 # Legend text size
...
### Soy Map ####################
#### Post-Sim Cropland Area ####################
terra::plot(r_row %>% subset("new_LND_SOY"),
            # type = "interval",
            # breaks = c(0, 1, 5, 10, 20, 25, 30, 35, 45, 50),
            type = "continuous",
            #breaks = c(0, 1, 5, 10, 20, 25, 30, 35, 45, 50),
            ...
            #col = brewer.pal(9, "YlGn"),
            main = paste("Post-Simulation Soy Area", pct_title),
            plg=list( # parameters for drawing legend
              title = "Area (ha) / Grid Cell",
              #title.cex = 2, # Legend title size
              #cex = 2 # Legend text size
...
#### Actual (Raw) Change in Cropland Area ####################
terra::plot(r_row %>% subset("rawch_SOY"),
            #type = "interval",
            #breaks = c(-50, -25, -10, -1, -0.1, 0, 0.01, 0.1, 0.25, 0.5, 1),
            #col = brewer.pal(n = 11, name = "RdBu"),
            ...
            main = paste("Raw Change in Soy Area", pct_title),
            plg=list( # parameters for drawing legend
              title = "Area (ha)/ Grid Cell",
              #title.cex = 2, # Legend title size
              #cex = 2 # Legend text size
...
# # PROBLEM HERE IS THAT WE STILL NEED TO MANUALLY CUT EACH DF
# F_ggplot_br_interval <- function(df, title_text, title_legend, save_title){
#   
#   # plot 
#   #"atlas", "high_relief", "arid", "soft", "muted", 
#   #"purple", "viridi", "gn_yl", "pi_y_g", "bl_yl_rd", "deep"
...
#   save_title = "gg_world_rawch_croplandarea.png")
# 
# F_ggplot_int_cut <- function(data, var, area){ 
...
#   pal <- colorRampPalette(brewer.pal(11, "YlGn"))
#   
#   # plot changes
#   p <- ggplot(data)+
...
# library(classInt)
# map_sty <- "pretty"
# (p_prod_diff_pct <- F_ggplot_int_cut(r_br, "new_QLAND", "Brazil"))
# getwd()
...
F_ggplot_brcerr(
  ...
  pal = "gn_yl", 
  legend_title = "Area (ha)",
  p_title = paste("Change in Soy Area", pct_title),
  save_title = "gg_br_soy_rawch_croparea_cont.png"
)
...
F_ggplot_brcerr(
  ...
  pal = "gn_yl", 
  legend_title = "Area (ha)",
  p_title = paste("Post-Sim Cerrado Cropland Area", pct_title),
  save_title = "gg_cerr_croplandarea.png")
...
F_ggplot_brcerr(
  ...
  pal = "gn_yl", 
  legend_title = "1000 Tons CE",
  p_title = paste("Post-Sim Change in Cerrado Crop Production", pct_title),
  save_title = "gg_cerr_cropproduction.png")
...
F_ggplot_brcerr(
  df = r_cerr %>% subset("new_LND_MAZ"),
  area = "Cerrado",
  brks = waiver(),
  pal = "gn_yl", 
  legend_title = "Area (ha)",
  p_title = paste("Post-Sim Maize Area", pct_title),
  save_title = "gg_cerr_maize_croparea.png")

#### Actual (Raw) Change in Cropland Area ####################
F_ggplot_brcerr(
  df = r_cerr %>% subset("rawch_MAZ"),
  area = "Cerrado",
  brks = waiver(),
  pal = "gn_yl", 
  legend_title = "Area (ha)",
  p_title = paste("Change in Maize Area", pct_title),
  save_title = "gg_cerr_maize_rawch_croparea_cont.png"
)
...
#### Post-Sim Soy Land #####################################
F_ggplot_brcerr(
  df = r_cerr %>% subset("new_LND_SOY"),
  area = "Cerrado",
  brks = waiver(),
  pal = "gn_yl", 
  legend_title = "Area (ha)",
  p_title = paste("Post-Sim Soy Area", pct_title),
  save_title = "gg_cerr_soy_croparea.png")
...
F_ggplot_brcerr(
  ...
  legend_title = "Area (ha)",
  p_title = paste("Change in Soy Area", pct_title),
  save_title = "gg_cerr_soy_rawch_croparea_cont.png"
)
